...
## 2024-08-04 - Cache Intl.NumberFormat Instantiations
**Learning:** Instantiating `Intl.NumberFormat` is notoriously slow and CPU-intensive, taking ~7ms for 1000 iterations in Node.js, compared to ~0.9ms when reusing a cached instance. In React components, recreating these objects on every render pass (especially in loops like 60fps springs or chat message lists) causes significant main thread blocking and unnecessary garbage collection.
**Action:** Always extract and export static `Intl.NumberFormat` (and `Intl.Segmenter`/`Intl.DateTimeFormat`) instances to shared files (e.g., `core/formatters.ts`) for reuse across components. For dynamic formatters dependent on props (like `decimalPlaces`), use `useMemo` to scope instantiation to the component lifecycle and avoid recreation on every render.
