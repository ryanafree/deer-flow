...
## 2024-06-25 - Cache Intl formatter instances
**Learning:** Recreating `Intl` formatters (`Intl.NumberFormat`, `Intl.DateTimeFormat`) is an expensive operation in JavaScript. Creating them inside frequently called functions, render functions, or especially high-frequency callbacks like animation frame loops (`springValue.on("change")`) causes unnecessary O(N) memory allocation and iteration, slowing down execution.
**Action:** Always cache `Intl` formatter instances instead of recreating them on every render or function call. Use module-level constants for static configurations, or `useMemo` when configurations depend on component props.
